...
import { calculatePercentage, dashBoardBarChart, getInventoryCategories, } from "../utils/features.js";
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const latestTransactionPromise = Order.find({})
            ...
            .limit(4);
        const [thisMonthProducts, thisMonthUsers, thisMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, productCount, userCount, allOrdersTotal, lastSixMonthOrders, categories, femaleUsersCount, latestTransaction,] = await Promise.all([
        ...
        const Revenue = allOrdersTotal.reduce((total, order) => total + (order.total || 0), 0);
        const counts = {
            Revenue,
            user: userCount,
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const [processingOrder, shippedOrder, deliveredOrder, categories, productCount, productsOutOfStock, allOrders, allUsers, adminUSersCount, customerUsersCount,] = await Promise.all([
